...
class Quadrotor():
    ...
    # Plot contour
        self.plot_contour(contourPoints,randomHenonPoints)
        ...
        plt.pause(0.001)
    def plot_contour(self, contour_points,random_HenonPoints):
        ...
        self.ax.plot(x_random, y_random, z_random, 'y-',linewidth=0.4, label='trajectory')
...
def plotPlane(position, tangentialVector, length, width, initialHenonPoint):
    ...
    randomIteration = np.random.randint(0, len(henonPoints))
    randomHenonPoint = henonPoints[randomIteration]
    
    # Transform Henon points to the plane's local coordinate system
    localHenonPoints = [position + point[0] * referenceVector + point[1] * np.cross(referenceVector, tangentialVector) for point in henonPoints]
