...
class Quadrotor():
    ...
    # Plot contour
        self.plot_contour(contourPoints,randomHenonPoints,randomHenonPoints2,randomHenonPoints3,planes,henonPoints,building_faces)

        plt.xlim(-10, 10)
        plt.ylim(-10, 10)
        self.ax.set_zlim(-0.5,9)
        
        plt.pause(0.001)
    def plot_contour(self, contour_points,random_HenonPoints,randomHenonPoints2,randomHenonPoints3,planes,henonPoints,building_poly):
        # Unpack contour points
        # Unpack contour points
        x_contour, y_contour, z_contour = contour_points[:,0], contour_points[:,1], contour_points[:,2]
        plane = planes[6]
        self.ax.add_collection3d(Poly3DCollection([plane], color='g', alpha=0.1))
        x_random, y_random, z_random = random_HenonPoints[:,0], random_HenonPoints[:,1],random_HenonPoints[:,2]
        x_random2, y_random2, z_random2 = randomHenonPoints2[:,0], randomHenonPoints2[:,1],randomHenonPoints2[:,2]
        x_random3, y_random3, z_random3 = randomHenonPoints3[:,0], randomHenonPoints3[:,1],randomHenonPoints3[:,2]
          
        building_poly = Poly3DCollection(building_faces, facecolors='gray', linewidths=1, edgecolors='k', alpha=0.45)
        self.ax.add_collection3d(building_poly)

        points = henonPoints[6]
        points = np.array(points)
        self.ax.scatter(points[:, 0], points[:, 1], points[:, 2], color='orange', s=1, alpha=0.5)
# Plot the contour
        # Plot the contour
        ...
        self.ax.plot(x_random, y_random, z_random, 'r-', linewidth=1, label='trajectory 1', alpha=0.5)
        self.ax.plot(x_random2, y_random2, z_random2, color='orange', linewidth=1, label='trajectory 2', alpha=0.5)
        self.ax.plot(x_random3, y_random3, z_random3, color='purple', linewidth=1, label='trajectory 3', alpha=0.5)

        self.ax.grid(False)
        #self.ax.set_xlabel('X',fontsize ="25")
        #self.ax.set_ylabel('Y',fontsize ="25")
        #self.ax.set_zlabel('Z',fontsize ="25")
        
        # Optionally, you can add a legend and adjust limits if needed
        #self.ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.), ncol=1, frameon=True,fontsize="12")
...
pitch = 3.5      # Distance between successive turns
